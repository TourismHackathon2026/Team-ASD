...
export default function Chat() {
  ...
  const [history] = useState(['Everest Base Camp trip', 'Annapurna Circuit plan']);
  ...
  function addMsg(role, content) {
    setMessages(p => [...p, { role, content, time: now() }]);
  }
  ...
  async function sendImage() {
    addMsg('user', '📷 Identifying this image...');
    setLoading(true);
    const fd = new FormData();
    fd.append('image', imageFile);
    try {
      const { data } = await api.post('/ai/image', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  function startNewChat() {
    setMessages([{
      role: 'assistant',
      content: 'नमस्ते! New chat started. Ask me anything about Nepal!',
      time: now()
    }]);
  }
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 10px' }}>
          ...
          {history.map((h, i) => (
            <button key={i} style={{
              ...
              borderRadius: 10, marginBottom: 2, background: 'transparent',
              border: 'none', cursor: 'pointer', display: 'flex',
              alignItems: 'center', gap: 8, color: '#584237',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        ...
        <header style={{
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: '#8c7164', margin: 0 }}>Your Nepal travel expert • Replies in English & Nepali</p>
          </div>
